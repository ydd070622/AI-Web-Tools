    ...
    <div className="crm-page">
      <div className="crm-toolbar">
        <span className="crm-page-subtitle">
          已签合同 {activeContracts.length} 份 · 总金额 ¥{(total / 10000).toFixed(1)}万 · 已回款 ¥{(totalPaid / 10000).toFixed(1)}万{total - totalPaid > 0 ? <> · <span style={{ color: 'var(--danger, #ef4444)' }}>待收 ¥{((total - totalPaid) / 10000).toFixed(1)}万</span></> : ''}
        </span>
      ...
      {activeContracts.length === 0 ? <div className="crm-empty">暂无成交合同</div> : (
        <div className="crm-table-wrap">
          <table className="crm-table">
            <thead>
              <tr>
                {batchMode && <th style={{ width: 36 }}><input type="checkbox" checked={activeContracts.length > 0 && activeContracts.every(c => selectedIds.has(c.id))} onChange={e => { if (e.target.checked) setSelectedIds(new Set(activeContracts.map(c => c.id))); else setSelectedIds(new Set()) }} /></th>}
              ...
              {activeContracts.map(c => {
                ...
                return (
                  <tr key={c.id} onClick={() => { if (batchMode) toggleSel(c.id); else setViewingContract(c) }}>
                    ...
                    <td><button className="crm-btn-ghost-xs" style={{ color: 'var(--text-muted)' }} onClick={e => { e.stopPropagation(); if (confirm('归档后将移至「合同归档」，确定？')) archiveContract(c.id) }}>归档</button></td>
                  </tr>
                )
              })}
            </tbody>
